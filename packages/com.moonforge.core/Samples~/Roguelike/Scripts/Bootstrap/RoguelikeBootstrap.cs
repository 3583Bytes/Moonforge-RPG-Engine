...
namespace Moonforge.Sample.Roguelike
{
    ...
    public sealed class RoguelikeBootstrap : MonoBehaviour, IRoguelikeHost
    {
        ...
        [Tooltip("Camera background while in the town scene — warm dark brown reinforces the warm tile palette.")]
        [SerializeField] private Color _townBackgroundColor = new Color(0.14f, 0.10f, 0.06f, 1f);
        ...
        private void PaintMapTiles(MapRenderModel model)
        {
            ...
            for (int y = 0; y < map.Height; y++)
            {
                for (int x = 0; x < map.Width; x++)
                {
                    ...
                    {
                        // Walls bypass Unity's Tilemap and render as individual SpriteRenderer
                        // GameObjects. The Tilemap was placing wall tiles invisibly (likely
                        // chunk culling tiles with partial transparency); SpriteRenderers
                        // render reliably regardless.
        ...
        private void PaintWallSprite(int gridX, int gridY, TileVisualKind kind)
        {
            ...
            SpriteRenderer sr = go.AddComponent<SpriteRenderer>();
            sr.sprite = sprite;
            // sortingOrder 2: above the tilemap (default 0), below markers (5) and actors (10+).
            sr.sortingOrder = 2;
            _wallSprites.Add(go);
        }
