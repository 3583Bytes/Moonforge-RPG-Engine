...
- **Autotiled walls.** `GetWallSprite` picks a face from the 4-neighbour floor mask: a wall bordering a room to the south shows a brick front (`wall_mid`) and side walls use the vertical edge pieces (`wall_left` / `wall_right`). A dungeon wall that faces no room is left **unpainted** (the dark background shows through — the classic "only visible wall faces are drawn" look) rather than tiling a flat grey top over the whole wall body; town building bodies keep a top so they still read as solid.
...
- **Town ground.** The 0x72 set is dungeon-only, so the town uses two large seamless textures at the `Art/Resources/` root — `Grass.tga` and `Ground.tga`. Grass is a single **tiled ground plane** across the courtyard (seamless, not one texture crammed per cell). The dirt roads come from the **generator**: `TownLayout` **pathfinds** (BFS over walkable cells) from the central plaza to the doorstep of every building, so streets bend around building footprints and reach each entrance instead of ramming a wall. The result is real map data (exposed as `MapRenderModel.RoadCells`), not a shape faked over the top. The renderer paints those cells from `Ground.tga`, each sampling a continuous slice of the texture. Building walls, props, and characters stay 0x72. If `Grass.tga` is missing the town falls back to per-cell 0x72 floors.
