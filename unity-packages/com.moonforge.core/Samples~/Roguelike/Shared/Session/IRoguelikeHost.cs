...
namespace Moonforge.Sample.Roguelike.Session
{
    ...
    public interface IRoguelikeHost
    {
        ...
        /// <summary>
        /// Render a list-of-text overlay used by the Contract Journal, Shrine
        /// of Echoes, Boss Reward, and town interaction menus.
        /// </summary>
        void RenderContractJournal(string title, IReadOnlyList<string> lines, string controls);

        /// <summary>
        /// Render the gear loadout screen with structured slot + inventory
        /// data (replaces the earlier reuse of <see cref="RenderContractJournal"/>
        /// for gear, which crammed every item into a single text line).
        /// </summary>
        void RenderGearInventory(GearInventoryRenderModel model);
    }
}
