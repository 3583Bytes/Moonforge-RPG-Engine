...
namespace Moonforge.Sample.Roguelike.Input
{
    public sealed class PlayerInputAdapter
    {
        ...
        private static PlayerAction PollGearInventory()
        {
            ...
            {
                return PlayerAction.UnequipAll;
            }
            if (UnityEngine.Input.GetKeyDown(KeyCode.Tab) || UnityEngine.Input.GetKeyDown(KeyCode.F))
            {
                return PlayerAction.FilterCycle;
            }
            if (UnityEngine.Input.GetKeyDown(KeyCode.I))
            {
                return PlayerAction.Confirm;
        ...
        // Held-key variant: returns the direction whose key is currently down
        // (not just on the frame of press). Used by the bootstrap's
        // hold-to-continue path: after the hero's smooth-slide between cells
        // has caught up to its target, this fires the next MoveCommand if the
        // player is still holding a direction. Net effect — tap once for a
        // single cell, hold for continuous glide at ActorMoveSpeed cells/sec.
        public PlayerAction PollHeldMovement()
        {
